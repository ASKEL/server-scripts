...
###### CHECK ########
#
[[ ${#ROOTPASS} < 1 ]] && { echo "Error: ROOTPASS should be minimum 1 characters in length. "; exit 1;}
#
[[ ${#USERNAME} > 8 ]] && { echo "Error: USERNAME should be maximum 8 characters in length. "; exit 1;}
#
[[ ${#DOMAIN} < 4 ]] && { echo "Error: DOMAIN should be minimum 4 characters in length. "; exit 1;}
#
##############

echo "Creating user and home directory..."
useradd $USERNAME -m -G www-data -s "/bin/bash" -d "/var/www/$USERNAME"
if [ "$?" -ne 0 ]; then
	echo "Can't add user"
	exit 2
fi
...
mkdir /var/www/$USERNAME/build/develop
mkdir /var/www/$USERNAME/tmp
mkdir /var/www/$USERNAME/logs
ln -s /var/www/$USERNAME/build/develop /var/www/$USERNAME/www/web
...
php_admin_value[error_log] = /var/www/$USERNAME/logs/fpm-php.log
...
Q2="GRANT ALL PRIVILEGES ON $USERNAME.* TO $USERNAME@'%' IDENTIFIED BY '$MYSQLPASS' WITH GRANT OPTION;"
Q3="FLUSH PRIVILEGES;"
SQL="${Q1}${Q2}${Q3}"
